...
export class BunNodeModulesCollector extends NodeModulesCollector<BunDependency, BunDependency> {
  ...
  protected async getDependenciesTree(): Promise<BunDependency> {
    const rootManifest = await this.readPackageJson(this.rootDir)
    const rootName = rootManifest.name ?? "."

    const childMaps = await this.resolveChildren(
      this.rootDir,
  ...
  protected collectAllDependencies(tree: BunDependency): void {
    this.collectFromRecord(tree.dependencies)
    this.collectFromRecord(tree.optionalDependencies)
  }
  ...
  private collectFromRecord(entries?: Record<string, BunDependency>) {
    if (!entries) {
      return
    }
  ...
  private async loadDependency(alias: string, requesterDir: string, isOptional: boolean): Promise<BunDependency | null> {
    ...
    let realDir = installedPath
    try {
      realDir = await fs.realpath(installedPath)
    } catch {
      // ignore resolution errors and keep the original path so packaging can continue
    }
    ...
    try {
      const manifest = await this.readPackageJson(realDir)
